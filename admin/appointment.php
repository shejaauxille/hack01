...
<head>
    ...
    <style>
        ...
        /* New styles for the session column */
        .session-column {
...
<body>
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'approve' && isset($_GET['id'])) {
        $appoid = $_GET['id'];
        $sqlupdate = "UPDATE appointment SET appostatus='approved' WHERE appoid=?";
                                ...
                                <table width="93%" class="sub-table scrolldown" border="0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="table-headin">Service Booked</th>
                                            <th class="table-headin session-column" style="white-space: nowrap;">Session Day & Time</th>
                                    ...
                                    <tbody>
                                        <?php
                                        $sqlmain = "SELECT appointment.appoid, schedule.scheduleid, schedule.title, doctor.docname, patient.pname, schedule.scheduleday, schedule.timerange, appointment.apponum, appointment.appodate, appointment.appostatus FROM schedule INNER JOIN appointment ON schedule.scheduleid=appointment.scheduleid INNER JOIN patient ON patient.pid=appointment.pid INNER JOIN doctor ON schedule.docid=doctor.docid";
                                        ...
                                        $sqlmain .= " ORDER BY schedule.scheduleday DESC";
                                        ...
                                        if ($result->num_rows == 0) {
                                            echo '<tr>
                                                ...
                                                We couldn\'t find anything related to your keywords!</p>
                                            ...
                                            for ($x = 0; $x < $result->num_rows; $x++) {
                                                ...
                                                echo '<tr>
                                                    ...
                                                    <td class="session-column">
                                                        <span>'.substr($scheduleday,0,10).'</span><br>
                                                        <span style="font-weight: 700;">'.$timerange.'</span>
                                                    </td>
                                                    ...
                                                    if ($appostatus == 'approved') {
                                                        echo '<div style="display:flex;justify-content: center;">
                                                            <button class="btn-primary-soft btn" style="background-color: #28a745; color: white; padding: 12px 40px; border: none; cursor: default;">
                                                                <font class="tn-in-text">Approved</font>
                                                            </button>
                                                        </div>';
                                                    } else {
                                                        echo '<div style="display:flex;justify-content: center;">
                                                            <a href="?action=approve&id='.$appoid.'&name='.urlencode($pname).'&session='.urlencode($title).'&apponum='.urlencode($apponum).'" class="non-style-link">
                                                                <button class="btn-primary-soft btn button-icon" style="padding-left: 40px; padding-top: 12px; padding-bottom: 12px; margin-top: 10px;">
                                                                    <font class="tn-in-text">Approve</font>
                                                                </button>
                                                            </a>
                                                            &nbsp;&nbsp;&nbsp;
                                                            <a href="?action=drop&id='.$appoid.'&name='.urlencode($pname).'&session='.urlencode($title).'&apponum='.urlencode($apponum).'" class="non-style-link">
