 ...
 <div class="container">
     ...
     <table class="menu-container" border="0">
        ...
        <?php
                ...
                $sqlmain = "
  SELECT schedule.scheduleid, schedule.title, doctor.docname,
         schedule.scheduleday, schedule.timerange, schedule.nop
  FROM schedule
  INNER JOIN doctor ON schedule.docid = doctor.docid
  ORDER BY schedule.scheduleday ASC, schedule.title ASC
";
                $sqlpt1="";
                $insertkey="";
                $q='';
    ...
    $sqlmain = "
      ...
      FROM schedule
      INNER JOIN doctor ON schedule.docid = doctor.docid
      WHERE (doctor.docname LIKE '%$keyword%'
         OR schedule.title LIKE '%$keyword%'
         OR schedule.scheduleday LIKE '%$keyword%')
      ORDER BY schedule.scheduleday ASC, schedule.title ASC
    ";
    $insertkey = $keyword;
                            ...
                            <?php
                                ...
                                for ( $x=0; $x<($result->num_rows);$x++){
                                    ...
                                    for($q=0;$q<3;$q++){
                                        ...
                                        $title=$row["title"];
                                        $docname=$row["docname"];
                                        // Prefer scheduleday/timerange (new), fall back to scheduledate/timerange (legacy)
                                        $scheduleday = isset($row['scheduleday']) && trim($row['scheduleday']) != '' 
                                                ? $row['scheduleday'] 
                                                : (isset($row['scheduleday']) ? date('l', strtotime($row['scheduleday'])) : 'TBD');

                                        $timerange = isset($row['timerange']) && trim($row['timerange']) != '' 
                                                        ? $row['timerange'] 
                                                        : (isset($row['timerange']) ? date('h:i A', strtotime($row['timerange'])) . ' to ' . date('h:i A', strtotime($row['timerange'] . ' +5 hours')) : 'TBD');
                                        ...
                                        if(count($times) == 2){
                                            ...
                                            // Clean the end time:
                                            // Remove any trailing AM/PM from 24-hour time, e.g., "18:00PM" -> "18:00"
